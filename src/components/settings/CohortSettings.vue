...
const cohorts = computed({
  ...
  set: (val) => {
    val.forEach((v) => {
      const activity = activityStore.currentPageActivities.find(
        (a) => a.id === v.programActivityId,
      )
...
const saveCohortHandler = async (cohortData: any) => {
  try {
    const selectedProgramActivity = activityStore.currentPageActivities.find(
      (a) => a.id === cohortData.programActivityId,
    )
    if (!selectedProgramActivity)
      throw new Error(
        `Serviço com ID ${cohortData.programActivityId} não encontrado.`,
      )
...
const toggleStatusHandler = async (row: any) => {
  try {
    ...
    const updated = await cohortStore.updateCohortLifeCycleStatus(
      row.uuid,
      novoStatus,
    )
...
<template>
  <EditableTable
    ...
    @save="
      (row, { resolve, reject }) =>
        saveCohortHandler(row).then(resolve).catch(reject)
    "
    @delete="
      (row, { resolve, reject }) =>
        deleteCohortHandler(row.uuid).then(resolve).catch(reject)
    "
    @search="onSearch"
